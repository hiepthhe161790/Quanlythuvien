	...
	$(document).ready(function(){
		...
		function checkPhone(){
			...
			if($("#phone").val().length == 0 || !intRegex.test($("#phone").val())){
				$("#phoneerror").html("Vui lòng nhập vào số điện thoại hợp lệ");
				phone_error = true;
				$("#phoneerror").show(); 
			}
			else{
		...
		function checkEmail(){
			...
			if($("#mail").val().length == 0 || !emailPattern.test($("#mail").val())){
				$("#emailerror").html("Vui lòng nhập vào email hợp lệ");
				email_error = true;
				$("#emailerror").show(); 
			}
			else{
		...
		$("#register").click(function(){
			...
			if(!email_error && !phone_error && !name_error && !address_error){
				...
				var phone = $("#phone").val();
				var email = $("#mail").val();
				var sex = $("#sex").val(); // Changed from .text() to .val()
